...
export class AsocomunalService {
  ...
  /**
   * Procesa la inserción masiva desde un array de objetos JSON mapeados desde Excel.
   *
   * @param data Array de asocomunales.
   * @returns Un resumen de la carga indicando éxitos y errores.
   */
  async importMasivo(data: any[]): Promise<any> {
    const resumen = {
      total: data.length,
      validas: 0,
      errores: 0,
    ...
    if (!data || data.length === 0) return resumen;

    // Precargar municipios para mapeo rápido (sin tildes, case insensitive)
    ...
    municipios.forEach((m) => {
      municipioMap.set(normalizar(m.nombre), m.id);
    });

    for (let i = 0; i < data.length; i++) {
      const fila = data[i];
      const numFila = fila.no || (i + 1); // Intentamos usar el registro original, o la iteración
      try {
        const nombreMunicipio = fila.municipio?.toString().trim();
        const nombreAsocomunal = fila.asocomunal?.toString().trim();

        if (!nombreMunicipio) {
          throw new BadRequestException('El campo MUNICIPIO está vacío');
        }
        if (!nombreAsocomunal) {
          throw new BadRequestException('El campo ASOCOMUNALES está vacío');
        }

        const idMunicipio = municipioMap.get(normalizar(nombreMunicipio));
        if (!idMunicipio) {
          throw new NotFoundException(`El municipio '${nombreMunicipio}' no existe en el sistema`);
        }

        const estadoObj = fila.estadoJAC?.toString().trim().toLowerCase();
        let estadoBool = true;
        if (estadoObj === 'inactiva' || estadoObj === 'inactivo' || estadoObj === 'falso' || estadoObj === 'false') {
          estadoBool = false;
        }

        const createDto: CreateAsocomunalDto = {
          nombre: nombreAsocomunal,
          estado: estadoBool,
          municipioId: idMunicipio,
          presidente: fila.nombrePresidente?.toString() || null,
          telefono: fila.numeroContacto?.toString() || null,
          correo: fila.correoElectronico?.toString() || null,
        };

        // this.create validará duplicados y notificará a RabbitMQ por cada registro
        ...
        resumen.detalles.push({
          fila: numFila,
          asocomunal: fila.asocomunal || 'Fila ' + numFila,
          error: error.message || 'Error interno al guardar',
        });
      }
    }
